...
from utils.seq_dataset import SeqRecDataset
...
class TrainDataLoader(AbstractDataLoader):
    ...
    """
    def __init__(self, config, dataset, n_items, batch_size=1, shuffle=False, seq_dict=None):
        ...
        self.seq_train_dataset = SeqRecDataset(self.seq_dict['user_seq'], self.n_item, data_type='train')
        #self.train_sampler = RandomSampler(self.seq_train_dataset)
    ...
    def _get_neg_sample(self):
        ...
        # to tensor
        user_tensor = torch.tensor(u_ids.values).type(torch.LongTensor).to(self.device)
        item_tensor = torch.tensor(cur_data[self.config['ITEM_ID_FIELD']].values).type(torch.LongTensor).to(self.device)

        # Get sequences and answers for the users in the batch
        seqs = []
        answers = []
        for user in u_ids.values:
            # seq = self.seq_train_dataset[self.seq_train_dataset.user_ids.index(user)]
            # seqs.append(seq[1])  # Sequence tensor
            # answers.append(seq[2])  # Answer tensor
            _, seq_tensor, ans_tensor, _, _ = self.seq_train_dataset[user]
            #seq_tensor, ans_tensor = self.seq_train_dataset.user_to_seq[user]
            seqs.append(seq_tensor)
            answers.append(ans_tensor)

        # Stack sequences and answers
        seq_tensor = torch.stack(seqs).to(self.device)  # Shape: (batch_size, max_len)
        answer_tensor = torch.stack(answers).to(self.device)  # Shape: (batch_size,)

        batch_tensor = torch.cat((torch.unsqueeze(user_tensor, 0),
                                  torch.unsqueeze(item_tensor, 0),
                                  seq_tensor.transpose(0,1),
                                  torch.unsqueeze(answer_tensor,0)))
    ...
    def _get_non_neg_sample(self):
        cur_data = self.dataset[self.pr: self.pr + self.step]
        self.pr += self.step
        user_ids = cur_data[self.config['USER_ID_FIELD']].values
        # to tensor
        user_tensor = torch.tensor(user_ids).type(torch.LongTensor).to(self.device)
        item_tensor = torch.tensor(cur_data[self.config['ITEM_ID_FIELD']].values).type(torch.LongTensor).to(self.device)
        # Get sequences and answers for the users in the batch
        seqs = []
        answers = []
...
class EvalDataLoader(AbstractDataLoader):
    ...
    """
    def __init__(self, config, dataset, n_item, additional_dataset=None,
                 batch_size=1, shuffle=False, seq_dict=None, valid=True):
        ...
        self.seq_dict = seq_dict
        if self.valid:
            self.seq_eval_dataset = SeqRecDataset(self.seq_dict['user_seq'], self.n_item, data_type='valid')
        else:
            self.seq_eval_dataset = SeqRecDataset(self.seq_dict['user_seq'], self.n_item, data_type='test')

        self.user_id_to_index = {}
        for idx, u_id in enumerate(self.seq_eval_dataset.user_ids):
            # u_id is the enumerated ID from the dataset
            # idx is the integer index in the dataset
            self.user_id_to_index[u_id] = idx

        # Create a mapping from user_id to index in SeqRecDataset
        # self.user_id_to_index = {user_id: idx for idx, user_id in enumerate(self.seq_eval_dataset.user_ids)}
    @property
    def pr_end(self):
    ...
    def _next_batch_data(self):
        ...
        # # Get the indices of the batch users in SeqRecDataset
        # batch_user_indices = [self.user_id_to_index[u.item()] for u in batch_users]
        #
        # # Extract sequences and answers for the batch users
        # batch_data = [self.seq_eval_dataset[idx] for idx in batch_user_indices]
        # batch_sequences = [data[1] for data in batch_data]  # input_ids
        # batch_answers = [data[2] for data in batch_data]  # answers

        # -----------------------------------------------------------------
        # 4) Convert each user ID to a dataset index, then call __getitem__
        # -----------------------------------------------------------------
        # Because in SeqRecDataset, user_ids[index] = enumerated user_id
        # So we must find that 'index' that matches user_id.
        batch_indices = []
        for u in batch_users:
            user_id = u.item()  # int
            # If user_id is enumerated from 0..(num_users-1),
            # we have a direct dictionary from user_id -> dataset index
            idx = self.user_id_to_index[user_id]
            batch_indices.append(idx)

        # Now fetch each example from the dataset
        # Each example is a 5-tuple:
        #   ( user_id, input_ids, answer, neg/zeros, sem_aug/zeros )
        batch_data = [self.seq_eval_dataset[i] for i in batch_indices]

        # 5) Unpack them
        user_ids_list, seqs_list, answers_list, neg_list, semaug_list = zip(*batch_data)

        # # Convert lists to tensors
        # batch_sequences = torch.stack(batch_sequences).to(self.device)
        # batch_answers = torch.tensor(batch_answers, dtype=torch.long).to(self.device)

        # 6) Convert them to Tensors on the current device
        #    (some are already Tensors, but we can re-wrap or move to device)
        user_ids_tensor = torch.stack(user_ids_list).to(self.device)
        seqs_tensor = torch.stack(seqs_list).to(self.device)
        answers_tensor = torch.stack(answers_list).to(self.device)

        return [batch_users, batch_mask_matrix, seqs_tensor, answers_tensor]
