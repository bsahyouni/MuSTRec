...
class Config(object):
    ...
    def __init__(self, model=None, dataset=None, config_dict=None, mg=False):
        ...
        if 'valid_metric' not in self.final_config_dict:
            raise KeyError("Expected 'valid_metric' in the configuration but it was not found.")
    ...
    def _load_dataset_model_config(self, config_dict, mg):
        ...
        cur_dir = os.getcwd()
        #cur_dir = os.path.join(cur_dir, 'src')  # Add 'src' to the path
        ...
        for file in file_list:
            print(f"Checking if file exists: {file}")  # Debug: Check file paths
            if os.path.isfile(file):
                print(f"Reading file: {file}")  # Debug: Confirm files are being read
